...
class TimeConstraintWeekly(AbstractConstraint):
    """
    ...
    def init(self, info: dict):
        ...
        self._lower_bound: datetime.timedelta = datetime.timedelta(days=date.weekday(), hours=date.hour, minutes=date.minute, seconds=date.second)
        date = info['UPPER_BOUND']
        self._upper_bound: datetime.timedelta = datetime.timedelta(days=date.weekday(), hours=date.hour, minutes=date.minute, seconds=date.second)

    def check(self, date: datetime.datetime):
        if self._start_date <= date < self._end_date:
            timedelta = datetime.timedelta(days=date.weekday(), hours=date.hour, minutes=date.minute, seconds=date.second)
