...
class InventoryDAO(AbstractDAO, SingletonInstance):
    ...
    def select_route_inventory(self, session: AbstractSession, **params):
        """
        세션 인스턴스를 통해 Data Source로부터 리스트 데이터를 조회
        :param session: AbstractSession 인스턴스
        :param params: sql 파라미터 데이터
        ...
        return session.select(
            """
                select *
                from FS_INV
                where INV_ID IN (select CURR_LOC_ID from FS_ROUTE where sim_id = :simulation_id and CURR_LOC_ID_TYP = 'INV')
                   OR INV_ID IN (select NEXT_LOC_ID from FS_ROUTE where sim_id = :simulation_id and NEXT_LOC_ID_TYP = 'INV')
            """, params)

    def select_route_item(self, session: AbstractSession, **params):
        """
        세션 인스턴스를 통해 Data Source로부터 리스트 데이터를 조회
        :param session: AbstractSession 인스턴스
        :param params: sql 파라미터 데이터
    ...
    def select_route_item_constraint(self, session: AbstractSession, **params):
        """
        세션 인스턴스를 통해 Data Source로부터 리스트 데이터를 조회
        :param session: AbstractSession 인스턴스
        :param params: sql 파라미터 데이터
